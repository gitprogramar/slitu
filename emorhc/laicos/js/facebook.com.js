...
chrome.runtime.onMessage.addListener(function(request, sender, callback) {
	if(window.location.host.indexOf('facebook.com') == -1) 
		return;
	if(request.action == 'facebook_alreadyLoaded')
		callback({alreadyLoaded: true});
	else if(request.action == 'facebook_load')
		contentScriptFacebook.load();
	     
	return true;  /* Will respond asynchronously */
});
	
contentScriptFacebook.load = function() {
