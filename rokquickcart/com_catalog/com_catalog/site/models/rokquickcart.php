...
class RokQuickCartModelRokQuickCart extends JModelList
{
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		$this->setState('list.limit', 0); //$this->setState('list.limit', $value);
		

		//$value = $app->getUserStateFromRequest($this->context.'.limitstart', 'limitstart', 0);
		$value = JFactory::getApplication()->input->getUint('limitstart', 0);
		$this->setState('list.start', 0); //$this->setState('list.start', $value);
	...
	{
		// filter by alias
		$alias = trim(strtolower(JFactory::getApplication()->getMenu()->getActive()->alias));
		// filter by id
		if(!empty($_GET["filter"])) {
			$productId = $_GET["filter"];
		}
		...
		$where = "published = 1";		
		$where .= " AND LOWER(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(a.params, '{\"', -1), '\":',1), '\/', ' '), '\\\', '')) = '" . $alias . "'";
		if(isset($productId)) {
