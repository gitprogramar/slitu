<?php
/**
 * @version   $Id: default.php 28481 2015-05-27 13:17:43Z james $
...
<div class="cart cart_quickcart cart_columns_<?php echo $this->cols; ?>" data-rqc-columns="<?php echo $this->cols; ?>">
	...
	<?php
		...
		if(!empty($_GET["collection_status"])) {
			...
			if($status == "rejected" && isset($_SESSION["refreshPage"]) && $_SESSION["refreshPage"] === false) {  ?>
				<script>program.notify("El pago fué rechazado", "Por favor intente comprar nuevamente", "fail");</script>
	  <?php } 
	...
	<div class="cart_cartstatus">
		<a onclick="program.scroll('#rokquickcart');"><?php echo JText::_('ROKQUICKCART_YOUR_CART');?>: <span class="simpleCart_quantity"></span> <?php echo JText::_('ROKQUICKCART_ITEMS');?></a>
	</div>		
	...
	<div class="cart_products<?php echo $this->same_height ? ' cart_same_height': ''; ?>">
		<?php
		// filter products
		$result = from($this->items)
		->where(function($item){ 
			if(!empty($_GET["filter"])):
				if($item->id == $_GET["filter"]):
					return true;
				endif;			
			elseif ($item->custom_fields): 
				foreach($item->custom_fields as $field_type => $field_html): 
					if(trim(strtolower(JFactory::getApplication()->getMenu()->getActive()->title)) == trim(strtolower($field_type))):
						return true;	
					endif;
				endforeach;					
			endif;
		...
		//echo $this->pagination->get('pages.total'); 
		//var_dump($this->items->count());
		//var_dump($stock);
		//var_dump($this->items);
				...
				<div class="cart_product_content">
					...
					<div class="column-pad cart_product_r <?php if(!empty($_GET["filter"])): echo "cart_product_r_filter"; endif;?>">
						...
						<div class="cart_product_add">
							<input id="<?php echo "cart_index_" . $index; ?>" type="hidden" value="<?php echo $item->id; ?>" />
		...
		<?php 
			...
			if(empty($_GET["filter"])) { // only for item list
				$json_ld = rtrim($json_ld, ",");
				$json_ld .= "]";
			}
			if ($result->count() == 0) {			
				$activeMenu = JFactory::getApplication()->getMenu()->getActive();
				$childMenus = JFactory::getApplication()->getMenu()->getItems('parent_id', $activeMenu->id);				
				// show sub categories
				if($childMenus && count($childMenus) > 0) {
					echo "<p style=\"font-size: 1.3em;padding: 3% 5% 2% 5%;\">Seleccione una categoría</p>";
					echo "<ul style=\"font-size: 1.3em;padding: 0% 5% 0% 5%;\">";
			...
			<?php				 
					...
					echo "<p style=\"font-size: 1.3em;padding: 3% 5% 1% 1%;;\">No hay artículos para mostrar.</p>";
				}
			} else {
			?>
...
<!-- Pagination 
<?php if ($this->pagination->get('pages.total') > 1 && $result->count() > 0) : ?>
...
<div class="clr"></div>
	...
	<a id="rokquickcart"></a>	
	<h3 class="cart-title"><?php echo JText::_('ROKQUICKCART_YOUR_CART');?>:
		(<span class="simpleCart_quantity"></span> <?php echo JText::_('ROKQUICKCART_ITEMS');?>)</h3>
	...
	<div class="cart_yourcart">
		<div class="cart_yourcart_items">
			...
			<div class="cart_buttons column-pad">				
				...
				<?php if(JComponentHelper::getParams('com_rokquickcart')->get('checkout_local') == 1) :?>
				<div id="btnCheckout" class="btn btn-success"><i class="icon-cart"></i> En el local<span></span></div>
				<?php endif; ?>
				<?php if(JComponentHelper::getParams('com_rokquickcart')->get('checkout_transfer') == 1) :?>
				<div id="btnCheckoutBank" class="btn btn-info"><i class="icon-share"></i> Transferencia Bancaria<span></span></div>
				<?php endif; ?>
				<?php if(JComponentHelper::getParams('com_rokquickcart')->get('checkout_mercadopago') == 1) :?>
				<div id="btnCheckoutMercadoPago" class="btn btnImageBootstrap"><span>Mercado Pago</span></div>
				<?php endif; ?>
				<?php if(JComponentHelper::getParams('com_rokquickcart')->get('checkout_paypal') == 1) :?>
				<div id="btnCheckoutPaypal" class="btn btnImageBootstrap"><span>PayPal</span></div>
				<?php endif; ?>
			</div>
		</div>
	</div>
...
$(document).ready(function() {	
	...
	$("#btnCheckoutMercadoPago").on("click", function() {
		...
		if(<?php echo json_encode($this->shipping); ?> == "1") {
	...
	// checkout En el local
	$("#btnCheckout").on("click", function() {
		processCheckout("local");
	});
	
	// checkout transferencia
	$("#btnCheckoutBank").on("click", function() {
		processCheckout("transfer");
	});
...
function processCheckout(paymentType) {
	if(simpleCart.quantity() == 0) {
		...
		$.ajax({
			type: "POST",
			data: { cart: JSON.stringify(items), text: emailTxt, payment: paymentType }, 
